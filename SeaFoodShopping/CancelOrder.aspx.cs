...
public partial class CancelOrder : System.Web.UI.Page
{
    ...
    protected void Page_Init(object sender, EventArgs e)
    {
        ...
        {
            //InsertLogAuditTrail(Session["sesUserID"].ToString(), "CancelOrder", "Page_Init", ex.ToString(), "Audit");
    ...
    public DataTable DataSourceHelper(string orderno,string Runningno)
    {
        ...
        string sql = "select * FROM VW_OrderDetails WHERE Deleted=0 and buy=1 and status=1 and Deliver=0 and Shipped=0 and Customerid='" + Session["CustomerID"].ToString() + "' and RefNo='" + orderno.ToString() + "' and RunningNo='" + Runningno.ToString() + "' and created_date between '" + nextday.ToString() + "' and '" + today.ToString() + "'";
    ...
    protected void btnCancelOrder_Click(object sender, EventArgs e) 
        {
            ...
            try
            {
                ...
                string sql = "select * FROM VW_OrderDetails WHERE Deleted=0 and buy=1 and status=00 and Deliver=0 and Shipped=0 and Customerid='" + Session["CustomerID"].ToString() + "' and orderid='" + @Param.ToString() + "' and RunningNo='" + @Param2.ToString() + "' and created_date between '" + nextday.ToString() + "' and '" + today.ToString() + "'";
                ...
                if (reader1.Read())
                {
                    ...
                    {
                        lblStatus.Text = "This Product is already Canceled";
                        return;
                    }
                }
                reader1.Close();
                ...
                int flg = BusinessTier.AddCartReturnCancel(conn, @Param.ToString(), @Param2.ToString().Trim(), cboReason.Text.ToString(), cboreimburse.Text.ToString(), cboBankers.Text.ToString(), txtAccname.Text.ToString(), txtAccno.Text.ToString(), txtBankname.Text.ToString(), txtBranch.Text.ToString(), Session["CustomerID"].ToString().Trim(), "C");
                    BusinessTier.DisposeConnection(conn);
